//
//  CommonLead&ReportViewController.swift
//  NewMleadsEnterprise
//
//  Created by Triforce New Mac on 06/02/20.
...
class SalesCycleReportDetailViewController: UIViewController,NVActivityIndicatorViewable {

    @IBOutlet var lblNumberOfLeads: UILabel!
    @IBOutlet var lblLeadsConverted: UILabel!
    @IBOutlet var lblAvgExpConvert: UILabel!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.title = "Sales Cycle Report"
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "EmailReport" , style: .plain, target: self, action: #selector(emailReport))
        
//        //Webservices CAll...
//        let param:[String : AnyObject] = ["userId":objLoginUserDetail.createTimeStamp! as AnyObject,"eventId":selectedId as AnyObject,"type":TypeId as AnyObject]
//       //MARK: Progress Bar ....
//        let size = CGSize(width: 30, height: 30)
//        startAnimating(size, message: "Loading...", type: NVActivityIndicatorType(rawValue: 29))
//        webService.doRequestPost(Sales_Cycle_Report_API_URL, params: param as [String : AnyObject], key: "lead", delegate: self)
    ...
    func callWebService()
    {
        let param:[String : AnyObject] = ["userId":objLoginUserDetail.createTimeStamp! as AnyObject,"eventId": objSelectedEvenId.createdTimeStamp as AnyObject,"type": "1" as AnyObject]
    ...
    @objc func emailReport()  {
        print("Email ")
        let param = ["template_code":"SALES_CYCLE_REPORT_MOBILE","userId":objLoginUserDetail.createTimeStamp!,"":"video_id"]
        //MARK: Progress Bar ....
        let size = CGSize(width: 30, height: 30)
        startAnimating(size, message: "Loading...", type: NVActivityIndicatorType(rawValue: 29))
...
extension SalesCycleReportDetailViewController:WebServiceDelegate{
...
//            print(data)
            ...
            lblNumberOfLeads.text = result["totalLeads"] as? String
            lblLeadsConverted.text = result["convertedLeads"] as? String
           // lblAvgExpConvert.text = (result["avgDats"] as? String)
            lblAvgExpConvert.text = String(format: "%.2f", (result.value(forKeyPath: "avgDats") as? NSNumber)?.floatValue ?? 0.0)
            print(result)
        }
...
extension SalesCycleReportDetailViewController: MFMailComposeViewControllerDelegate{
