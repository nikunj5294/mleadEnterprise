//
//  SalesCycleReportViewController.swift
//  NewMleadsEnterprise
//
...
class SalesCycleReportViewController: UIViewController,NVActivityIndicatorViewable {
    ...
    var selectedIDForEvent = String()
    var selectedIndexForEvent = Int()
    var selectedEventUserID = String()
    var arrEventCreatedTimeStemp = [String]()
    var selecteEventObj: EventDetail =  EventDetail()
    
    var typeID = String()
    var arrEventList = NSMutableArray()
    var eventListArr : NSMutableArray = NSMutableArray()
    ...
    @IBAction func btnGenerateReportAction(_ sender: Any) {
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "SalesCycleReportDetailViewController") as! SalesCycleReportDetailViewController
...
//MARK:- Webservices DElegate MEthod Call....
extension SalesCycleReportViewController:WebServiceDelegate{
