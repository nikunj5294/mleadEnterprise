//
//  ProductIntrestbyPercentageViewController.swift
//  NewMleadsEnterprise
//
...
class ProductIntrestbyPercentageViewController: UIViewController,NVActivityIndicatorViewable {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.title = "Product Intrest by Percentage"
    ...
    @IBAction func btnGenerateReportAction(_ sender: Any) {
       ...
       // let higt = self.view.frame.size.height-140
        ...
        let urlRequest: String? = "\(request)leadbyproduct.php?eventId=\(selecteEventObj.createdTimeStamp!)&userId=\(objLoginUserDetail.createTimeStamp!)&height=\(self.view.frame.size.height-140)&width=\(wdht)"
...
extension ProductIntrestbyPercentageViewController: WebServiceDelegate{
    ...
    func webServiceResponceSuccess(_ response: Data, apiKey: String) {
        ...
        else if apiKey == GET_EventList_For_User_Api
        {
            ...
            /*if arrEventList.count >= 0 {
                var objEvent = EventDetail()
                //objEvent = arrEventList(In)
                if (objEvent.createdTimeStamp == nil )
                {
                    if (objEvent.isGroup != nil)
                    {
                        self.lblEventGroup.text = "---GroupList"
                    }
                    else
                    {
                        self.lblEventGroup.text = "---EventList---"
                    }
