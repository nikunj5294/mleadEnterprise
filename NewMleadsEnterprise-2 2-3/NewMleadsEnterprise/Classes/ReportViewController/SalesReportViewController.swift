...
class SalesReportViewController: UIViewController,NVActivityIndicatorViewable {
    ...
    //MARK:- TeamMember BUtton Action Method...
    ...
    @IBAction func btnGenerateReportAction(_ sender: Any) {
        ...
        let request   = ReportURL + GET_Sales_Report + objLoginUserDetail.createTimeStamp!
        
        let urlRequest: String = "\(request)&height=\(self.view.frame.size.height-140)&width=\(self.view.frame.size.width)"
        vc.Url = urlRequest
        vc.isSales = true
...
extension SalesReportViewController:WebServiceDelegate{
